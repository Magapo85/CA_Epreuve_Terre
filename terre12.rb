...
#fonction utile qui test si l'argument est un nombre entier, ne contient pas . ou , et bien entre 0 et 23 ou 0 et 59 selon heure ou minute
def fauxEntier(nombre, type)
        if trouverDansArray(",", nombre)[1] == "erreur" || trouverDansArray(".", nombre)[1] == "erreur" || (nombre != "00" && nombre.to_i == 0) 
                return "erreur"
        else
		if (type == "heure" && (nombre.to_i < 0 || nombre.to_i > 23)) || (type == "minute" && (nombre.to_i < 0 || nombre.to_i > 59))
                	return "erreur"
		else 
			return "ok"
		end
...
if nombreArgument != 1 || longueurArgument(ARGV[0]) != 5 || ARGV[0][2] != ":" || fauxEntier(ARGV[0][0]+ARGV[0][1], "heure") == "erreur" || fauxEntier(ARGV[0][3]+ARGV[0][4], "minute") == "erreur"
	puts "erreur, saisir un format 24 h type : XX:XX"
elsif (ARGV[0][0] + ARGV[0][1]).to_i < 12 #ARGV[0][5] == "A"
	if ARGV[0][0] == "0" && ARGV[0][1] == "0"
        ...
        elsif ARGV[0][0] == "2" && ARGV[0][1].to_i < 2
		puts (ARGV[0][0].to_i - 2).to_s + (ARGV[0][1].to_i + 8).to_s + ":" + ARGV[0][3] + ARGV[0][4] + "PM"
	else
                puts (ARGV[0][0].to_i - 1).to_s + (ARGV[0][1].to_i - 2).to_s + ":" + ARGV[0][3] + ARGV[0][4] + "PM"
        end
else 
	puts "erreur, saisir un format 24 h type : XX:XX"
end
